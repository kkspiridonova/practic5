...
namespace pr5.ViewModel
{
    internal class MainViewModel : BindingHelper
    {
        ...
        private ObservableCollection<Users> us;
        public ObservableCollection<Users> Us
        {
            get { return us; }
            set
            {
        ...
        public MainViewModel()
        {
            ...
            Us = Json.Des<ObservableCollection<Users>>("us.json") ?? new ObservableCollection<Users>();
            Users = new ObservableCollection<Users>();
            Naz = Json.Des<ObservableCollection<string>>("naz.json") ?? new ObservableCollection<string>();
            repeat_write();
            Selected_index = -1;
            Index = -1;
            VibData();
        ...
        void Write()
        {
            ...
            if (go && Selected_index != -1 && Text_summ != null && Text_summ != "" && Name_record != null && Name_record != "")
            {
                try
                {
                    ...
                    {
                        Users.Add(new Users(Dt, Name_record, Naz[Selected_index], Convert.ToInt32(Text_summ), true));
                        Us.Add(new Users(Dt, Name_record, Naz[Selected_index], Convert.ToInt32(Text_summ), true));
                    }
                    else
                    {
                        Users.Add(new Users(Dt, Name_record, Naz[Selected_index], Convert.ToInt32(Text_summ), false));
                        Us.Add(new Users(Dt, Name_record, Naz[Selected_index], Convert.ToInt32(Text_summ), false));
                    }
                    Json.Ser("us.json", Us);
                    repeat_write();
                }
        ...
        {
            Users.Clear();
            if (Us.Count > 0) foreach (Users user in Us) if (time.Date == user.data.Value.Date) Users.Add(user);
        }
    }
}
